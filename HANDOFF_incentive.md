...
### Phase 4＋帰属設計の変更（インセンティブ売上の直接登録）実装内容（2026-07-28）
運用実態（選手が売るとは限らず、帰属は入力時に決めたい）を受けて、**売上入力時の選手指定を主経路**に変更。

- `/incentive/sales/new`：インセンティブ売上登録フォーム（月次集計の「＋ 売上を登録」から）
  - 売れ方（会場販売／手売り／EC販売）→ 選手 → 商品（カラー・サイズ）→ 数量・単価 を選ぶと**インセンティブ額をリアルタイム表示**（ルール解決を含む。`lib/incentiveRuleResolve.ts` に純関数として分離しクライアントと共用）
  - 登録すると**通常の売上実績として計上され在庫が減る**（variant.stockQuantity 減算・soldQuantity 加算＝既存ルール踏襲）。会場販売は大会への任意紐付け可、ECはEC売上一覧にも表示
- `SalesRecord.channel` に `"hand"`（手売り）を追加。`handSaleReported`（Lark申請済み）も追加
  - 手売り＝全グッズ対象10%・帰属は売った選手本人。**申請チェックなしで登録すると売上・在庫のみ計上され、集計では「未申請の手売り」として対象外表示**
- **帰属ルールの変更**：`wrestlerOverrideId` が明示された売上は、商品の区分（personal/multi/org/未設定）と紐付けを**無視して指定選手に100%帰属**（全チャネル）。商品×選手の事前紐付けは、選手指定なしで登録された売上（既存の売上フォーム・大会・EC経由）のフォールバックとして機能する
- 検証：手売り10%（¥7,000→¥700）・マッハ隼人80%粗利（¥4,000→¥3,200）のプレビュー、登録→在庫30→28→27、未申請手売りの対象外サマリ表示まで実操作で確認済み

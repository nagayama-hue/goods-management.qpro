...
| # | 事項 | 暫定実装 |
|---|---|---|
| 6 | 社員割（employee_discount）をインセンティブ対象とするか | 対象のまま仮置き。Phase 2 の対象判定関数の1条件で切替可能にする |
| 7 | 販売単価の税込/税抜の統一（¥3,182 商品の存在） | unit_price=実売単価（税込前提）で計算。運用側で単価の税込統一を確認 |
| 8 | マッハ隼人を選手マスタに追加するか（初期リスト漏れ疑い） | シードに含めず。UIから追加可能 |

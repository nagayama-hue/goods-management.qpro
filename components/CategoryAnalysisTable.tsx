...
import type { CategoryAggregate } from "@/lib/aggregations";
...
export default function CategoryAnalysisTable({ rows }: Props) {
  if (rows.length === 0) {
    return (
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50 text-xs text-gray-500">
            ...
            <th className="px-3 py-2 text-right font-medium">総売上</th>
            <th className="px-3 py-2 text-right font-medium">総粗利</th>
            <th className="px-3 py-2 text-right font-medium">平均粗利率</th>
            <th className="px-3 py-2 text-right font-medium">総製作数</th>
            <th className="px-3 py-2 text-right font-medium">総販売数</th>
            <th className="px-3 py-2 text-right font-medium">総在庫数</th>
            <th className="px-3 py-2 text-right font-medium">平均在庫率</th>
          ...
          {rows.map((row) => (
            <tr key={row.category} className="hover:bg-gray-50">
              <td className="px-3 py-2.5">
                <Link
                  href={`/analytics/category/${encodeURIComponent(row.category)}`}
              ...
              <td className="px-3 py-2.5 text-right tabular-nums text-gray-600">
                {formatNumber(row.totalProductionCount)}
              </td>
              <td className="px-3 py-2.5 text-right tabular-nums text-gray-600">
                {formatNumber(row.totalSalesCount)}
              </td>
              <td className="px-3 py-2.5 text-right tabular-nums text-gray-600">

...
import type { ChannelAggregate } from "@/lib/aggregations";
import { formatCurrency, formatMargin, formatNumber } from "@/lib/format";

interface Props {
  rows: ChannelAggregate[];
}
...
export default function ChannelAnalysisTable({ rows }: Props) {
  if (rows.length === 0) {
    return (
  ...
  return (
    <div>
            ...
            {rows.map((row) => (
              <tr key={row.channel} className="hover:bg-gray-50">
                <td className="px-3 py-2.5">
                  <Link
                    href={`/analytics/channel/${encodeURIComponent(row.channel)}`}
                ...
                <td className="px-3 py-2.5 text-right tabular-nums text-gray-600">
                  {row.avgSellingPrice > 0 ? formatCurrency(row.avgSellingPrice) : "—"}
                </td>
      ...
      <p className="px-3 py-2 text-xs text-gray-400">
        ※ 会場+EC 商品は会場・EC の両方に重複集計されます。各チャネルの合計は全体総計と一致しません。
      </p>
    </div>
  );
}

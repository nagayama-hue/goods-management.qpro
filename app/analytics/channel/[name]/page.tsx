...
import { getChannelAnalysis } from "@/lib/aggregations";
import type { PrimaryChannel } from "@/lib/aggregations";
...
const PRIMARY_CHANNELS = new Set<PrimaryChannel>(["会場", "EC", "FC限定"]);

function isPrimaryChannel(v: string): v is PrimaryChannel {
  return PRIMARY_CHANNELS.has(v as PrimaryChannel);
}

/** 商品が指定チャネルを含むか判定 */
function includesChannel(g: GoodsCalculated, channel: PrimaryChannel): boolean {
  if (g.salesChannel === channel) return true;
  if (g.salesChannel === "会場+EC" && (channel === "会場" || channel === "EC")) return true;
  return false;
}
...
function GoodsRow({ g }: { g: GoodsCalculated }) {
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-4 py-2.5 text-gray-600">{g.category}</td>
      <td className="px-4 py-2.5">
...
export default async function ChannelDetailPage({ params }: Props) {
  ...
  if (!isPrimaryChannel(channel)) notFound();

  const allGoods     = getAllGoods().map(calcGoods);
  const channelGoods = allGoods.filter((g) => includesChannel(g, channel));

  if (channelGoods.length === 0) notFound();

  const analysis = getChannelAnalysis(allGoods).find((r) => r.channel === channel);
  if (!analysis) notFound();

  const hitGoods  = channelGoods.filter((g) => evaluateGoods(g).level === "OK");
  const warnGoods = channelGoods.filter((g) => evaluateGoods(g).level === "WARN");
  const ngGoods   = channelGoods.filter((g) => evaluateGoods(g).level === "NG");
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-semibold text-gray-900">{channel}</h1>
        <p className="mt-1 text-sm text-gray-500">チャネル別分析</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          { label: "売上",      value: formatCurrency(analysis.revenue) },
          { label: "粗利",      value: formatCurrency(analysis.totalGrossProfit) },
          { label: "粗利率",    value: formatMargin(analysis.avgGrossMargin) },
          { label: "平均単価",  value: analysis.avgSellingPrice > 0 ? formatCurrency(analysis.avgSellingPrice) : "—" },
        ].map(({ label, value }) => (

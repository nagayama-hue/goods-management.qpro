...
import { getMonthlyAnalysis } from "@/lib/aggregations";
...
export default async function MonthDetailPage({ params }: Props) {
  ...
  const monthGoods  = allGoods.filter((g) => (g.releaseDate ?? "未設定") === month);

  if (monthGoods.length === 0) notFound();

  const analysis = getMonthlyAnalysis(allGoods).find((r) => r.month === month);
  if (!analysis) notFound();

  const hitGoods  = monthGoods.filter((g) => evaluateGoods(g).level === "OK");
  const warnGoods = monthGoods.filter((g) => evaluateGoods(g).level === "WARN");
  const ngGoods   = monthGoods.filter((g) => evaluateGoods(g).level === "NG");
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-semibold text-gray-900">{month}</h1>
        <p className="mt-1 text-sm text-gray-500">月別分析（発売月ベース）</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          { label: "在庫率",   value: formatMargin(analysis.avgInventoryRate) },
        ].map(({ label, value }) => (

...
| `/stocktake` 配下 | 棚卸し | 実数入力（スマホ最適化）・差異記録・確定で在庫反映・履歴・差異CSV |
...
| `stocktakes.json` | 棚卸し（下書き＋確定履歴。確定時に variant.stockQuantity を実数で上書き） |
...
### 5. EC企画紐付けの改善（担当者からの不具合報告対応）
**報告内容**: 企画管理で作った企画に売上明細が反映されない（原因は本番が紐付け機能実装前の旧コードだったこと＋入力済み明細を後から紐付ける手段がなかったこと）。

- 売上編集画面（`/sales/[id]/edit`）に「EC企画紐付け」欄を追加（channel=ec のレコードのみ表示）。**入力済みの明細を後から企画に紐付け・解除できる**
- EC売上登録フォームで企画を選ぶと、販売種別=企画販売・企画名を自動セット（手入力済みの企画名は上書きしない）
- 企画名欄に「実績集計は EC企画紐付けで決まる（名称は分類用）」の説明文を追加
- 選択できる企画が0件のとき、企画作成ページへの案内を表示
- 紐付けセレクトに出るのは**対象月が直近3ヶ月以降の企画**（編集時は現在紐付いている企画も表示）
